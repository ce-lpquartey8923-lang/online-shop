...
-- Run this in the Supabase SQL Editor. It creates only missing catalog tables
-- and does not delete or replace existing data.
...
begin
  ...
  loop
    ...
    if not found then
      ...
      values (category_record.name, category_record.slug);
    end if;
  end loop;
end $$;
...
where public.products.slug = catalog.slug;
...
select catalog.slug, catalog.name, categories.id, catalog.price, catalog.description, catalog.image_url, catalog.details, catalog.badge
from catalog
join public.categories on categories.slug = catalog.category_slug
where not exists (
  select 1 from public.products existing where existing.slug = catalog.slug
);
