...
function Account(){
  ...
  const submit=async(e:FormEvent)=>{
    e.preventDefault()
    if(!supabase){setMessage('Supabase is not configured. Add your environment variables and restart the app.');return}
  ...
  if(userEmail)return <main className="account"><div className="account-head"><div><p className="eyebrow">Welcome back</p><h1>Your account.</h1></div><button className="text-link" onClick={()=>supabase?.auth.signOut()}>Sign out</button></div><div className="account-grid"><section><h2>Order history</h2><p>Your orders will appear here after checkout.</p></section><aside className="account-aside"><h3>Signed in as</h3><p>{userEmail}</p></aside></div></main>
  return <main className="auth"><div className="auth-art"><img src="https://images.unsplash.com/photo-1549490349-8643362247b5?auto=format&fit=crop&w=1200&q=85" alt="Abstract still life"/><p>“The best objects are the ones that become part of your story.”</p></div><div className="auth-form"><Link to="/" className="wordmark">loam <i>&</i> linen</Link><div className="auth-tabs"><button className={mode==='signin'?'active':''} onClick={()=>{setMode('signin');setMessage('')}}>Sign in</button><button className={mode==='signup'?'active':''} onClick={()=>{setMode('signup');setMessage('')}}>Create account</button></div><h1>{mode==='signin'?'Welcome back.':'Make yourself at home.'}</h1><p>{mode==='signin'?'Access your orders and saved details.':'Keep track of orders and save your favorites.'}</p><form onSubmit={submit}>{mode==='signup'&&<input required value={fullName} onChange={e=>setFullName(e.target.value)} placeholder="Full name"/>}<input required type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email address"/><input required minLength={6} type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Password"/><button className="button button-dark full" disabled={submitting}>{submitting?'Please wait...':mode==='signin'?'Sign in':'Create account'} <ArrowRight size={16}/></button></form>{message&&<p role="alert">{message}</p>}</div></main>
}
