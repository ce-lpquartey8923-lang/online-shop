...
If the products are missing from an existing database, run `supabase/seed.sql` in the SQL Editor instead. It upserts all eight products and finishes with a verification query. The existing database must have `public.categories` and `public.products` with the columns used by the seed (`slug`, `name`, `category_id`, `price`, `description`, `image_url`, `details`, and `badge`). If Supabase reports that a table or column is missing, apply the relevant table definitions from `supabase/migrations/001_store.sql` first.
